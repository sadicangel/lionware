...
{
    /// <summary>
    /// The <see cref="DbfField" /> corresponding to <see langword="null"/>.
    /// </summary>
    public static readonly DbfField Null = new();

    // type code of the value.
    [FieldOffset(0)] private readonly TypeCode _type;
    // true if value is stored locally; false if it's a value on the heap.
    [FieldOffset(4)] private readonly bool _isInline;
    // size in bytes of the value (0 when not stored locally).
    [FieldOffset(5)] private readonly byte _inlineSize;
    // reserved.
    ...
    public DbfField(string? value)
    {
        ...
        _type = value is not null ? TypeCode.String : TypeCode.Empty;
    }

    /// <summary>
    /// Gets a value indicating whether this instance is <see langword="null" />.
    /// </summary>
    public readonly bool IsNull { get => _type is TypeCode.Empty; }

    internal readonly TypeCode Type { get => _type; }
    ...
    {
        if (_type is not TypeCode.Boolean)
    ...
    public int GetInt32()
    {
        if (_type is not TypeCode.Int32)
    ...
    {
        if (_type is not TypeCode.Int64)
    ...
    {
        if (_type is not TypeCode.Double)
    ...
    {
        if (_type is not TypeCode.DateTime)
    ...
    {
        if (_type is not TypeCode.String)
